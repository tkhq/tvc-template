...
| `--no-input` | `TVC_NO_INPUT` | Disable all interactive prompts. Auto-enabled when stdin is not a TTY. |
...
The template contains `<FILL_IN_...>` placeholders that MUST be replaced before creating the app. The operator public key is auto-populated from login.
...
| `--manifest-id` | `TVC_MANIFEST_ID` | Manifest UUID (required with `--manifest`) |
| `--operator-id` | `TVC_OPERATOR_ID` | Operator UUID (auto-used from config if omitted) |
| `--operator-seed <PATH>` | | Custom operator key file |
| `--yes` / `-y` | | Skip all interactive approval prompts |
| `--dry-run` | | Review manifest without generating approval |
| `--skip-post` | | Don't post approval to API |
| `--output` / `-o` | | Write approval to file |
...
# 5. App is live at:
echo "https://app-${APP_ID}.turnkey.cloud"
```
